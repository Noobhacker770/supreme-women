...
function RoomShell({ activity, state, setState, onBack, children }: {
  ...
  const finishWorld = () => {
    if (state.completed.includes(activity.id)) return;
    setState(prev => completeRoom(prev, activity.id, activity.collectible));
  };
  ...
  useEffect(() => {
    if (!showComplete || autoCancelled || !next) return;
    const timer = window.setTimeout(continueNext, 2600);
    return () => window.clearTimeout(timer);
  }, [showComplete, autoCancelled, next?.id]);
  ...
  return <div className={`room-page theme-${activity.theme}`}>
    ...
    <main className="room-content">
      ...
      {children}
      {!done && <div className="room-fallback-complete">
        <span>Everything in this room is optional.</span>
        <button className="secondary" onClick={finishWorld}>FINISH WORLD <Check size={15}/></button>
      </div>}
    </main>
    {showComplete && <div className="completion-overlay" role="dialog" aria-modal="true">
      <div className="completion-card">
        ...
        <p><Sparkles size={15}/> <b>{activity.collectible}</b> added to your universe.</p>{next && <small className="auto-next-note">Next world opens automatically in a moment.</small>}
        <div className="completion-actions">
          ...
          <button className="secondary" onClick={() => { setAutoCancelled(true); setShowComplete(false); }}>STAY HERE</button>
        </div>
      </div>
    </div>}
  </div>;
...
function BonkRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
  ...
  return <RoomShell activity={activity} state={state} setState={setState} onBack={onBack}>
...
function CafeRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
...
function GardenRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
...
function CloudRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
...
function CozyRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
  ...
  return <RoomShell activity={activity} state={state} setState={setState} onBack={onBack}><div className="cozy-room"><div className="window"><div className="moon-window"/><span>2:17 AM</span></div><div className="room-furniture"><div className="sofa"/><div className="table"/><div className="rug"/></div>{objects.map((o,i)=><button key={o} className={`hidden-object object-${o} ${found.includes(i)?'found':''}`} onClick={()=>click(i)}>{found.includes(i)?<Check/>:o==='cat'?<Cat/>:<Sparkles/>}</button>)}<div className="cozy-caption">{found.length}/5 little things found · {found.length===5?'The room feels warmer.':'Look carefully. The quiet things are usually hiding.'}</div></div></RoomShell>;
}

function ArcadeRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
  ...
  return <RoomShell activity={activity} state={state} setState={setState} onBack={onBack}><div className="arcade-room"><div className="arcade-cabinet"><div className="screen"><div className="scanline"/><span className="score-big">{score.toString().padStart(2,'0')}</span>{started?<button className="neon-target" style={{left:`${12+((active*37)%72)}%`,top:`${16+((active*53)%66)}%`}} onClick={()=>{setScore(v=>v+1);setActive(v=>v+1)}}><CircleDot/></button>:<span className="insert">INSERT COURAGE</span>}</div><div className="controls"><span>TIME {seconds}</span><span>NEON ARCADE // 198X</span><span>HIGH {Math.max(score, state.stars)}</span></div></div><button className="mega-action neon" onClick={start}>{started?'KEEP GOING':'START GAME'} <Zap/></button></div></RoomShell>;
}

function DressRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
  ...
  return <RoomShell activity={activity} state={state} setState={setState} onBack={onBack}><div className="dress-room"><div className="runway"><div className="spotlight-cone"/><Avatar mood="fashion"/><div className="runway-label">LOOK {String(state.outfit.length).padStart(2,'0')}</div></div><div className="wardrobe"><div className="panel-label">WARDROBE / SELECT ANYTHING</div>{pieces.map(x=><button className={state.outfit.includes(x)?'selected':''} key={x} onClick={()=>toggle(x)}>{x}{state.outfit.includes(x)&&<Check/>}</button>)}<button className="mega-action" disabled={state.outfit.length<3} onClick={()=>setState(prev=>completeRoom(prev,activity.id,activity.collectible))}>LOCK THE LOOK <Shirt/></button></div></div></RoomShell>;
}

function CarnivalRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
...
function MusicRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
  ...
  return <RoomShell activity={activity} state={state} setState={setState} onBack={onBack}><div className="music-room"><div className="music-orbit">{Array.from({length:8},(_,i)=><button key={i} className={melody.includes(i)?'lit':''} onClick={()=>tap(i)} style={{transform:`rotate(${i*45}deg) translateY(-150px) rotate(-${i*45}deg)`}}>{notes[i]}</button>)}<div className="music-core"><Music2/></div></div><div className="music-console"><button className="secondary" onClick={generate}>GENERATE MELODY</button><button className="primary" onClick={()=>setPlaying(!playing)}>{playing?<Pause/>:<Play/>}{playing?'PLAYING':'PLAY'}</button><p>{sequence.length?`Sequence: ${sequence.map(i=>notes[i]).join(' · ')}`:'Make your own tiny song.'}</p></div></div></RoomShell>;
}

function ChocolateRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
  ...
  return <RoomShell activity={activity} state={state} setState={setState} onBack={onBack}><div className="choco-room"><div className="choco-lab"><div className="conveyor"><div className="choco-piece" style={{'--coating':coating} as React.CSSProperties}>{decor==='Star'?'★':decor==='Heart'?'♥':'✦'}</div><div className="conveyor-line"/></div><div className="choco-brief"><span>ATELIER // 10</span><h2>{finished?'Approved by the Chocolate Council':'Design your signature piece.'}</h2><p>Three decisions. No wrong answers. Several suspiciously fancy ones.</p></div></div><div className="three-columns"><div><small>SHAPE</small>{['Moon','Heart','Square'].map(x=><button className={selected===x?'selected':''} onClick={()=>setSelected(x)} key={x}>{x}</button>)}</div><div><small>COATING</small>{['Ruby','Dark','Vanilla'].map(x=><button className={coating===x?'selected':''} onClick={()=>setCoating(x)} key={x}>{x}</button>)}</div><div><small>DECOR</small>{['Star','Heart','Spark'].map(x=><button className={decor===x?'selected':''} onClick={()=>setDecor(x)} key={x}>{x}</button>)}</div></div><button className="mega-action" onClick={finish}>SEND TO QUALITY CONTROL <Check/></button></div></RoomShell>;
}

function PetRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
...
function DanceRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
  ...
  return <RoomShell activity={activity} state={state} setState={setState} onBack={onBack}><div className="dance-room"><div className="stage-floor"><div className="spotlight"/><Avatar mood={step%2?'dance':'proud'}/><div className="scoreboard">SCORE <b>{score}</b></div></div><div className="dance-console"><div className="beat-line">{pattern.map((p,i)=><span key={i} className={i<step?'hit':i===step?'next':''}>{pads[p]}</span>)}</div><div className="dance-pads">{pads.map((x,i)=><button key={x} onClick={()=>hit(i)}>{x}</button>)}</div><small>Follow the glowing sequence.</small></div></div></RoomShell>;
}

function MysteryRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
...
function ForbiddenRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
  ...
  return <RoomShell activity={activity} state={state} setState={setState} onBack={onBack}><div className="button-room"><div className={`forbidden-core p${Math.min(5,Math.floor(press/3))}`}><div className="warning-ring"/><button onClick={pressButton}><Zap/><span>DO NOT<br/>PRESS</span></button></div><div className="button-terminal"><span>SECURITY EVENT LOG</span><div>{Array.from({length:6},(_,i)=><p key={i}><b>0{i+1}:17:4{i}</b> {i<press?`USER ACTION DETECTED // ${alert}`:'AWAITING USER DECISION'}</p>)}</div><strong>{press.toString().padStart(2,'0')} / 15</strong></div></div></RoomShell>;
}

function QuizRoom({ state, setState, activity, onBack }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack: () => void }) {
...
function MoonRoom({ state, setState, activity, onBack, onFinal }: { state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; activity: Activity; onBack:()=>void; onFinal:()=>void }) {
  ...
  return <RoomShell activity={activity} state={state} setState={setState} onBack={onBack}><div className="moon-room"><div className="observatory"><div className="moon-orb"><div/></div><div className="constellation-lines">{connected.slice(1).map((x,j)=>{const a=points[connected[j]],b=points[x];return <span key={x} style={{left:`${a.x}%`,top:`${a.y}%`,width:`${Math.hypot((b.x-a.x)*9,(b.y-a.y)*9)}px`,transform:`rotate(${Math.atan2(b.y-a.y,b.x-a.x)*180/Math.PI}deg)`}}/>})}</div>{points.map((p,i)=><button key={i} className={connected.includes(i)?'connected':''} style={{left:`${p.x}%`,top:`${p.y}%`}} onClick={()=>connect(i)}><Star fill="currentColor"/></button>)}</div><div className="moon-copy"><span>OBSERVATORY // LAST CONSTELLATION</span><h2>{done?'The hidden pattern is complete.':'There is a shape hiding in the stars.'}</h2><p>{done?'Something just unlocked somewhere you have not seen before.':'Connect every star. No timer. Just curiosity.'}</p>{done&&<button className="mega-action moon" onClick={onFinal}>OPEN THE DOOR <ChevronRight/></button>}</div></div></RoomShell>;
}

function GenericRoom({ activity, state, setState, onBack }: { activity: Activity; state: GameState; setState: React.Dispatch<React.SetStateAction<GameState>>; onBack:()=>void }) {
  return <RoomShell activity={activity} state={state} setState={setState} onBack={onBack}><div className="generic-room"><div className="generic-emblem">{activity.icon}</div><h2>{activity.title}</h2><p>{activity.subtitle}</p><button className="mega-action" onClick={()=>setState(prev=>completeRoom(prev,activity.id,activity.collectible))}>COMPLETE EXPEDITION <Sparkles/></button></div></RoomShell>;
}
...
function App() {
  ...
  if(active==='moon' && activity)return <MoonRoom activity={activity} state={state} setState={setState} onBack={()=>setActive(null)} onFinal={()=>setActive('final')} />;
  if(active && activity){
    const common={state,setState,activity,onBack:()=>setActive(null)};
    return active==='anger'?<BonkRoom {...common}/> : active==='cafe'?<CafeRoom {...common}/> : active==='garden'?<GardenRoom {...common}/> : active==='cloud'?<CloudRoom {...common}/> : active==='cozy'?<CozyRoom {...common}/> : active==='arcade'?<ArcadeRoom {...common}/> : active==='dress'?<DressRoom {...common}/> : active==='carnival'?<CarnivalRoom {...common}/> : active==='music'?<MusicRoom {...common}/> : active==='chocolate'?<ChocolateRoom {...common}/> : active==='pet'?<PetRoom {...common}/> : active==='dance'?<DanceRoom {...common}/> : active==='mystery'?<MysteryRoom {...common}/> : active==='compliment'?<ComplimentRoom {...common}/> : active==='button'?<ForbiddenRoom {...common}/> : active==='quiz'?<QuizRoom {...common}/> : active==='castle'?<CastleRoom {...common}/> : <GenericRoom {...common}/>;
  }
